...
import app.cn.extra.mall.merchant.vo.GetAreaCode;
import app.cn.extra.mall.merchant.vo.GetGuidePic;
...
public class SplashActivity extends BaseActivty implements AMapLocationUtils.AMapLocationCallBack {
    ...
    /**
     * 判断用户是否登陆过或账号是否通过审核，并根据判断结果跳转至对应界面
     */
    private void judgeUserLogged() {
        if (!TextUtils.isEmpty(sharePreferenceUtil.getUID())
                && !TextUtils.isEmpty(sharePreferenceUtil.getSChecked())) {
            jumpToMainActivity();
        } else {
            jumpToLoginActivity();
        }
                        ...
                        public void onResponse(String response, int id) {
                            ...
                            if (Constants.OK.equals(getAreaCode.getFlag())) {
                                if (Constants.OK.equals(getAreaCode.getData().getStatus())) {
                                    sharePreferenceUtil.setLocationAcId(getAreaCode.getData()
                                            .getAcId());
                                }
                            }
                        }
