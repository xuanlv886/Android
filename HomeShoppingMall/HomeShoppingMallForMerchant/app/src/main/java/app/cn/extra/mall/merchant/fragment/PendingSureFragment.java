...
import app.cn.extra.mall.merchant.activity.PendingSureDetailsActivity;
 ...
 * 待确认
 */
public class PendingSureFragment extends BaseFragment
    ...
    private CommonAdapter<GetUserRequirement.DataBean.StoreApplyRequirementListBean> adapter;
    /**
     * 当前页，用于分页获取数据 从1开始
     */
     ...
     * 用于存放临时数据
     */
    List<GetUserRequirement.DataBean.StoreApplyRequirementListBean> userRequirementBeanList = null;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        mView = inflater.inflate(R.layout.fragment_pending_sure, container, false);
...
//                DividerItemDecoration.VERTICAL));
    ...
    private void getUserRequirement() {
        if (Utils.isNetworkAvailable(MyDemandActivity.context)) {
            ...
            OkHttpUtils
                    ...
                    .addParams("roStatus", Constants.STATUS_DEMAND_PENDINGSURE)
                            ...
                            if (Constants.OK.equals(getUserRequirement.getFlag())) {
                                if (loadMore) {
                                    if (null != getUserRequirement.getData()
                                            ...
                                            .getStoreApplyRequirementList().size()) {
                                        for (int i = 0; i < getUserRequirement.getData()
                                                .getStoreApplyRequirementList().size(); i++) {
                                            userRequirementBeanList.add(getUserRequirement.getData()
                                                    .getStoreApplyRequirementList().get(i));
                                        }
    ...
    private void setRecommendProductData() {
        adapter = new CommonAdapter<GetUserRequirement.DataBean.StoreApplyRequirementListBean>(
                MyDemandActivity.context, R.layout.list_item_demand, userRequirementBeanList) {
            @Override
            protected void convert(ViewHolder holder, GetUserRequirement.DataBean.StoreApplyRequirementListBean userRequirementBean, int position) {
                ...
                holder.setText(R.id.tv_demand_type, "待确认");
                ...
                if (userRequirementBean.getUrOfferType() == 1) {
                    holder.setText(R.id.tv_demand_money, "悬赏金额：" + "需商家报价");
                } else {
                    holder.setText(R.id.tv_demand_money, "悬赏金额：¥" + userRequirementBean.getUrOfferPrice());
                }
                if (userRequirementBean.getUrBrowserNum() > 0) {
                    holder.setText(R.id.tv_demand_num, userRequirementBean.getNum() + "个商家已申请接单");
                }
        ...
        adapter.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                Intent intent = new Intent(MyDemandActivity.context, PendingSureDetailsActivity.class);
