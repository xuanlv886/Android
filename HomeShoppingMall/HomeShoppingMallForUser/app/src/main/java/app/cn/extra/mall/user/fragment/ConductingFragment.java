...
import app.cn.extra.mall.user.activity.ConductingDetailsActivity;
...
import app.cn.extra.mall.user.vo.GetUserRequirement;
 ...
 * 进行中
 */
public class ConductingFragment extends BaseFragment
     ...
     * 用于存放临时数据
     */
    List<GetUserRequirement.DataBean.UserRequirementListBean> UserRequirementListBeanList = null;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mView = inflater.inflate(R.layout.fragment_conducting, container, false);
...
//                DividerItemDecoration.VERTICAL));
    ...
    private void getUserRequirement() {
        if (Utils.isNetworkAvailable(MyDemandActivity.context)) {
            ...
            OkHttpUtils
                    .post()
                    .url(Constants.getUserRequirement)
                    .addParams("status", Constants.STATUS_DEMAND_CONDUCTION)
                                ...
                                if (loadMore) {
                                    if (null != getUserRequirement.getData()
                                        ...
                                        for (int i = 0; i < getUserRequirement.getData()
                                            ...
                                            UserRequirementListBeanList.add(getUserRequirement.getData()
                                    ...
                                    if (null != getUserRequirement.getData()
                                            ...
                                            && 0 < getUserRequirement.getData()
                                            .getUserRequirementList().size()) {
                                        UserRequirementListBeanList = getUserRequirement.getData()
                                                .getUserRequirementList();
                                        if (UserRequirementListBeanList != null) {
    ...
    private void setRecommendProductData() {
        adapter = new CommonAdapter<GetUserRequirement.DataBean.UserRequirementListBean>(
                MyDemandActivity.context, R.layout.list_item_demand, UserRequirementListBeanList) {
            @Override
            protected void convert(ViewHolder holder, GetUserRequirement.DataBean.UserRequirementListBean UserRequirementListBean, int position) {
                if (!TextUtils.isEmpty(UserRequirementListBean.getUrTitle())) {
                    holder.setText(R.id.tv_demand_title,
                            "需求标题：" + UserRequirementListBean.getUrTitle());
                } else {
                    holder.setText(R.id.tv_demand_title, "需求标题：");
                }
                holder.setText(R.id.tv_demand_type, "进行中");
                holder.setText(R.id.tv_demend_content, "需求描述：" + UserRequirementListBean.getUrContent());
                holder.setText(R.id.tv_send_time, "发布时间：" + UserRequirementListBean.getUrCreateTime());
                holder.getView(R.id.tv_confirm_time).setVisibility(View.VISIBLE);
                holder.setText(R.id.tv_confirm_time, "确认时间：" + UserRequirementListBean.getRoConfirmTime());
                holder.setText(R.id.tv_demand_money, "悬赏金额：¥" + UserRequirementListBean.getUrOfferPrice());
                if ("取货送货".equals(UserRequirementListBean.getRtName())) {
                    holder.setVisible(R.id.ll_demand_get_address, true);
                    holder.setText(R.id.tv_demand_get_address, "取货地址：" + UserRequirementListBean.getUrGetAddress());
                } else {
        ...
        adapter.setOnItemClickListener(new MultiItemTypeAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                Intent intent=new Intent(MyDemandActivity.context, ConductingDetailsActivity.class);
                intent.putExtra("urId",UserRequirementListBeanList.get(position).getUrId());
