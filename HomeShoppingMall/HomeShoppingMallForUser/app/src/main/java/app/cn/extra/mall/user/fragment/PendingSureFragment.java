...
import app.cn.extra.mall.user.activity.PendingSureDetailsActivity;
import app.cn.extra.mall.user.event.PendingSureFragmentEvent;
    ...
    private void getUserRequirement() {
        if (Utils.isNetworkAvailable(MyDemandActivity.context)) {
            ...
            OkHttpUtils
                    ...
                    .addParams("status", Constants.STATUS_DEMAND_PENDINGSURE)
    ...
    private void setRecommendProductData() {
        adapter = new CommonAdapter<GetUserRequirement.DataBean.UserRequirementListBean>(
            ...
            protected void convert(ViewHolder holder, GetUserRequirement.DataBean.UserRequirementListBean userRequirementBean, int position) {
                ...
                if (1 == userRequirementBean.getUrOfferType()) {
                    holder.setText(R.id.tv_demand_money, "悬赏金额：需商家报价");
                } else {
                    holder.setText(R.id.tv_demand_money, "悬赏金额：¥"
                            + userRequirementBean.getUrOfferPrice());
                }

                if (userRequirementBean.getApplyNum() > 0) {
                    holder.setText(R.id.tv_demand_num, userRequirementBean.getApplyNum() + "个商家已申请接单");
                }
