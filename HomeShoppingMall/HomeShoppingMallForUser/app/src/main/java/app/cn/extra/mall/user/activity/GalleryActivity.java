...
import app.cn.extra.mall.user.utils.Bimp;
import app.cn.extra.mall.user.utils.ImageItem;
...
public class GalleryActivity extends BaseActivty {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        int adapterPos = intent.getIntExtra("adapterPos", 0);
        if (1 == position) { // 相册中预览 AlbumActivity
            bmps = (ArrayList<ImageItem>) Bimp.tempSelectBitmap.get(adapterPos).getList();
        } else if (2 == position) { // 已选择的图片预览 RelaseBannerActivity
            bmps = (ArrayList<ImageItem>) Bimp.SelectBitmap.get(adapterPos).getList();
