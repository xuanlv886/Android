...
public class RecentContactsFragment extends TFragment {
    ...
    public static RecentContactsFragment newInstance(String param1) {
...
//                getUserInfoToServer(recent.getContactId(), 0);
    ...
    private void showDialog(String toUid, int position) {
        LayoutInflater layoutInflater = LayoutInflater.from( com.netease.nim.uikit.merchant.Utils.getActivity());
        View view = layoutInflater.inflate(R.layout.dialog_set_memo_name, null);
        final Dialog alertDialog = new AlertDialog.Builder( com.netease.nim.uikit.merchant.Utils.getActivity(), R.style.Theme_Transparent).
                setView(view).
                create();
        alertDialog.show();
        ...
        tv_live_yes.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (TextUtils.isEmpty(memoName)) {
                    Toast.makeText(com.netease.nim.uikit.merchant.Utils.getActivity(), "请填写备注名！", Toast.LENGTH_SHORT).show();
                    return;
                }
                //为用户设置备注名
                setUserMemoName(alertDialog, Constants.MYUID, toUid, memoName, position);
            }
        });
    }
    ...
    private void setUserMemoName(final Dialog dialog, String fromUid, String toUid, String memoName, int position) {
        if (Utils.isNetworkAvailable(com.netease.nim.uikit.merchant.Utils.getActivity())) {
            OkHttpUtils
                    .post()
                        ...
                        public void onResponse(String response, int id) {
                            ...
                            if (Constants.OK.equals(setUserMemoName.getFlag())) {
                                if (Constants.OK.equals(setUserMemoName.getData().getStatus())) {
                                    ...
                                    Toast.makeText(com.netease.nim.uikit.merchant.Utils.getActivity(), "修改备注名成功！", Toast.LENGTH_SHORT).show();
                                    RecyclerView.ViewHolder viewHolder = recyclerView.findViewHolderForLayoutPosition(position);
                                    TextView tvName = viewHolder.itemView.findViewById(R.id.tv_nickname);
                                    tvName.setText(memoName);
                                } else {
                                    Toast.makeText(com.netease.nim.uikit.merchant.Utils.getActivity(),
            ...
            Toast.makeText(com.netease.nim.uikit.merchant.Utils.getActivity(), Constants.NETWORK_ERROR, Toast.LENGTH_SHORT).show();
        }
    }
                        ...
                        for (int i = 0; i < loadedRecents.size(); i++) {
                            ...
                            if (user != null) {
                                try {
                                    ...
                                    map.put("sType", jsonObject.getString("sType"));
